...
Appends the specified `content` at the `index` in the original string. If a range _ending_ with `index` is subsequently moved, the insert will be moved with it. Returns `this`. See also `s.prependLeft(# CODE_TRUNCATED)`.

### s.appendRight( index, content )

Appends the specified `content` at the `index` in the original string. If a range _starting_ with `index` is subsequently moved, the insert will be moved with it. Returns `this`. See also `s.prependRight(# CODE_TRUNCATED)`.
...
**DEPRECATED** since 0.17 – use `s.appendLeft(# CODE_TRUNCATED)` instead

### s.insertRight( index, content )

**DEPRECATED** since 0.17 – use `s.prependRight(# CODE_TRUNCATED)` instead
...
It may be preferred to use `s.update(# CODE_TRUNCATED)` instead if you wish to avoid overwriting the appended/prepended content.
...
Same as `s.appendLeft(# CODE_TRUNCATED)`, except that the inserted content will go _before_ any previous appends or prepends at `index`

### s.prependRight ( index, content )

Same as `s.appendRight(# CODE_TRUNCATED)`, except that the inserted content will go _before_ any previous appends or prepends at `index`
...
As an alternative syntax, if you a) don't have `filename` or `indentExclusionRanges` options, or b) passed those in when you used `new MagicString(# CODE_TRUNCATED)`, you can simply pass the `MagicString` instance itself:
