...
* `{key1: val1, key2: val2, # CODE_TRUNCATED}` is an Object, and the casting procedure is recursively applied to each property. Eg. `{a: 1, b: 2}` is `{a: 1, b: 2}`.
...
parse('RegExp', 're', {explicit: true});           // Error: # CODE_TRUNCATED does not type check# CODE_TRUNCATED
...
parse('Even', '3', options); // Error: Value: "3" does not type check# CODE_TRUNCATED
```
