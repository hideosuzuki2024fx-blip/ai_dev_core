...
myCar.hooks.calculateRoutes.intercept({
	...
	register: (tapInfo) => {
		// tapInfo = { type: "promise", name: "GoogleMapsPlugin", fn: # CODE_TRUNCATED }
...
**call**: `(# CODE_TRUNCATEDargs) => void` Adding `call` to your interceptor will trigger when hooks are triggered. You have access to the hooks arguments.

**tap**: `(tap: Tap) => void` Adding `tap` to your interceptor will trigger when a plugin taps into a hook. Provided is the `Tap` object. `Tap` object can't be changed.

**loop**: `(# CODE_TRUNCATEDargs) => void` Adding `loop` to your interceptor will trigger for each loop of a looping hook.
...
myCar.hooks.accelerate.intercept({
	context: true,
	tap: (context, tapInfo) => {
		// tapInfo = { type: "sync", name: "NoisePlugin", fn: # CODE_TRUNCATED }
...
myCar.hooks.accelerate.tap(
	...
	(context, newSpeed) => {
		if (context && context.hasMuffler) {
			console.log("Silence# CODE_TRUNCATED");
		} else {
			console.log("Vroom!");
		}
	}
...
```ts
interface Hook {
	tap: (name: string | Tap, fn: (context?, # CODE_TRUNCATEDargs) => Result) => void;
	tapAsync: (
		name: string | Tap,
		fn: (
			context?,
	...
	tapPromise: (
		name: string | Tap,
		fn: (context?, # CODE_TRUNCATEDargs) => Promise<Result>
	) => void;
	intercept: (interceptor: HookInterceptor) => void;
}

interface HookInterceptor {
	call: (context?, # CODE_TRUNCATEDargs) => void;
	loop: (context?, # CODE_TRUNCATEDargs) => void;
...
interface Hook {
	isUsed: () => boolean;
	call: (# CODE_TRUNCATEDargs) => Result;
	promise: (# CODE_TRUNCATEDargs) => Promise<Result>;
	callAsync: (
		# CODE_TRUNCATEDargs,
		callback: (err: Error | null, result: Result) => void
	) => void;
