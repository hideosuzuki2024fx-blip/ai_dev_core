...
### merge2(stream1, stream2, # CODE_TRUNCATED, streamN)

### merge2(stream1, stream2, # CODE_TRUNCATED, streamN, options)

### merge2(stream1, [stream2, stream3, # CODE_TRUNCATED], streamN, options)

return a duplex stream (mergedStream). streams in array will be merged in parallel.

### mergedStream.add(stream)

### mergedStream.add(stream1, [stream2, stream3, # CODE_TRUNCATED], # CODE_TRUNCATED)
...
`objectMode` and other options(`highWaterMark`, `defaultEncoding` # CODE_TRUNCATED) is same as Node.js `Stream`.
