...
* `.sync(# CODE_TRUNCATEDargs)` - Returns the computed value, or throws.
* `.async(# CODE_TRUNCATEDargs)` - Returns a promise for the computed value.
* `.errback(# CODE_TRUNCATEDargs, (err, result) => {})` - Calls the callback with the computed value, or error.
...
* `opts.async`
  ...
  Example: `async (# CODE_TRUNCATEDargs) => 4`
...
* `opts.errback`
  ...
  Example: `(# CODE_TRUNCATEDargs, cb) => cb(null, 4)`
