...
Changing this for example to `100` will make line `1` of the input source to be marked as line `100`, line `2` as `101`, line `3` as `102`, # CODE_TRUNCATED
...
Disallow import type { type a } from # CODE_TRUNCATED ([#305](https://github.com/babel/babylon/pull/305)) (Daniel Tschinder)
...
export const foo = 1;
export const [bar, # CODE_TRUNCATEDfoo] = baz;
```
...
```js
let { x, y, # CODE_TRUNCATEDz } =  { x: 1, y: 2, z: 3 };
// x = 1
// y = 2
// z = { z: 3 }
```
...
```bash
> 1 | let { # CODE_TRUNCATEDx, y, z } = { x: 1, y: 2, z: 3};
    |           ^
# Previous behavior:
# x = { x: 1, y: 2, z: 3 }
# y = 2
...
```bash
> 1 | let { x, # CODE_TRUNCATEDy, # CODE_TRUNCATEDz } = { x: 1, y: 2, z: 3};
    |              ^
# Previous behavior:
# x = 1
# y = { y: 2, z: 3 }
...
```js
<div>
  {# CODE_TRUNCATEDtodos.map(todo => <Todo key={todo.id} todo={todo}/>)}
</div>
```
...
- Fix parse errors when using arrow functions with an spread element and return type `(# CODE_TRUNCATEDprops): void => {}` ([#10](https://github.com/babel/babylon/pull/10)) @danez
