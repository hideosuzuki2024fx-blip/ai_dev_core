...
```javascript
var f = new Fraction("9.4'31'"); // 9.4313131313131# CODE_TRUNCATED
f.mul([-4, 3]).mod("4.'8'"); // 4.88888888888888# CODE_TRUNCATED
```
...
```javascript
Number(f.s) * Number(f.n) / Number(f.d) = -1 * 4154 / 1485 = -2.797306# CODE_TRUNCATED
```

If you attempted to calculate this manually using floating-point arithmetic, you'd get something like:

```javascript
(9.4313131 * (-4 / 3)) % 4.888888 = -2.797308133# CODE_TRUNCATED
```
...
---
It turns out that Fraction.js outperforms almost any fmod() implementation, including JavaScript itself, [php.js](http://phpjs.org/functions/fmod/), C++, Python, Java and even Wolframalpha due to the fact that numbers like 0.05, 0.1, # CODE_TRUNCATED are infinite decimal in base 2.
...
*Fraction.js* can easily handle repeating decimal places. For example *1/3* is *0.3333# CODE_TRUNCATED*. There is only one repeating digit. As you can see in the examples above, you can pass a number like *1/3* as "0.'3'" or "0.(3)", which are synonym. There are no tests to parse something like 0.166666666 to 1/6! If you really want to handle this number, wrap around brackets on your own with the function below for example: 0.1(66666666)
...
To automatically make a number like "0.123123123" to something more Fraction.js friendly like "0.(123)", I hacked this little brute force algorithm in a 10 minutes. Improvements are welcome# CODE_TRUNCATED
