...
# Functions for typechecking# CODE_TRUNCATED
...
def enforce_bytes(value: bytes | str, *, name: str) -> bytes:
    ...
    the plain ASCII range. chr(0)# CODE_TRUNCATEDchr(127). If you need to use characters
...
# Interfaces for byte streams# CODE_TRUNCATED
...
class URL:
    ...
    at `http://localhost:8080`# CODE_TRUNCATED
    ...
    Another example is constructing an `OPTIONS *` request# CODE_TRUNCATED
...
class Response:
    ...
    def iter_stream(self) -> typing.Iterator[bytes]:
        if not isinstance(self.stream, typing.Iterable):  # pragma: nocover
            raise RuntimeError(
                "Attempted to stream an asynchronous response using 'for # CODE_TRUNCATED in "
                "response.iter_stream()'. "
                "You should use 'async for # CODE_TRUNCATED in response.aiter_stream()' instead."
            )
        if self._stream_consumed:
            raise RuntimeError(
                "Attempted to call 'for # CODE_TRUNCATED in response.iter_stream()' more than once."
            )
    ...
    # Async interface# CODE_TRUNCATED
    ...
    async def aiter_stream(self) -> typing.AsyncIterator[bytes]:
        if not isinstance(self.stream, typing.AsyncIterable):  # pragma: nocover
            raise RuntimeError(
                "Attempted to stream an synchronous response using 'async for # CODE_TRUNCATED in "
                "response.aiter_stream()'. "
                "You should use 'for # CODE_TRUNCATED in response.iter_stream()' instead."
            )
        if self._stream_consumed:
            raise RuntimeError(
                "Attempted to call 'async for # CODE_TRUNCATED in response.aiter_stream()' "
                "more than once."
            )
        self._stream_consumed = True
