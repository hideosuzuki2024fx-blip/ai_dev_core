...
if sys.version_info >= (3, 14) or sys.version_info < (3, 10):
    ...
    def is_union_origin(obj: Any, /) -> bool:
        ...
        !!! note
            Since Python 3.14, both `Union[<t1>, <t2>, # CODE_TRUNCATED]` and `<t1> | <t2> | # CODE_TRUNCATED` forms create instances
...
def get_literal_values(
    ...
    *,
    ...
    Example:
        ...
        ```
    """
    # `literal` is guaranteed to be a `Literal[# CODE_TRUNCATED]` special form, so use
...
def inspect_annotation(  # noqa: PLR0915
    ...
    *,
    ...
    Example:
        ```pycon
        >>> inspect_annotation(
        # CODE_TRUNCATED     Final[Annotated[ClassVar[Annotated[int, 'meta_1']], 'meta_2']],
        # CODE_TRUNCATED     annotation_source=AnnotationSource.CLASS,
        # CODE_TRUNCATED )
        # CODE_TRUNCATED
...
def _unpack_annotated_inner(
    ...
    if check_annotated and typing_objects.is_annotated(origin):
        ...
        # unpack it. Because Python already flattens `Annotated[Annotated[<type>, # CODE_TRUNCATED], # CODE_TRUNCATED]` forms,
        ...
        # (e.g. with `type MyList[T] = Annotated[list[T], # CODE_TRUNCATED]`, `MyList[int]`
        # is a generic alias).
        try:
            ...
            # - With `MyList = Annotated[list[T], # CODE_TRUNCATED]`, `MyList[int] == Annotated[list[int], # CODE_TRUNCATED]`
            # - With `type MyList[T] = Annotated[list[T], # CODE_TRUNCATED]`, `MyList[int].__value__ == Annotated[list[T], # CODE_TRUNCATED]`.

            try:
                # To do so, we emulate the parameterization of the value with the arguments:
                # with `type MyList[T] = Annotated[list[T], # CODE_TRUNCATED]`, to emulate `MyList[int]`,
                # we do `Annotated[list[T], # CODE_TRUNCATED][int]` (which gives `Annotated[list[T], # CODE_TRUNCATED]`):
