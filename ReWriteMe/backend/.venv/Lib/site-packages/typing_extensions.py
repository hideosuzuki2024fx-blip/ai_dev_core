...
if sys.version_info >= (3, 10, 1):
    ...
    Literal = _LiteralForm(doc="""\
                           ...
                           Literal[# CODE_TRUNCATED] cannot be subclassed. There is no runtime
...
if hasattr(typing, "get_overloads"):  # 3.11+
    ...
    def overload(func):
        ...
        @overload
        def utf8(value: None) -> None: # CODE_TRUNCATED
        @overload
        def utf8(value: bytes) -> bytes: # CODE_TRUNCATED
        @overload
        def utf8(value: str) -> bytes: # CODE_TRUNCATED
...
if sys.version_info >= (3, 13):
    ...
    class _ProtocolMeta(type(typing.Protocol)):
        ...
        # but is necessary for several reasons# CODE_TRUNCATED
        #
    ...
    def _proto_hook(cls, other):
        ...
        for attr in cls.__protocol_attrs__:
            for base in other.__mro__:
                # Check if the members appears in the class dictionary# CODE_TRUNCATED
                if attr in base.__dict__:
                    if base.__dict__[attr] is None:
                        return NotImplemented
                    break

                # # CODE_TRUNCATEDor in annotations, if it is a sub-protocol.
                annotations = getattr(base, '__annotations__', {})
                if (
...
if hasattr(io, "Reader") and hasattr(io, "Writer"):
    ...
    class Reader(Protocol[T_co]):
        ...
        @abc.abstractmethod
        def read(self, size: int = # CODE_TRUNCATED, /) -> T_co:
...
if _PEP_728_IMPLEMENTED:
    ...
    class _TypedDictMeta(type):
        ...
        def __new__(cls, name, bases, ns, *, total=True, closed=None,
            ...
            if ns_annotations is not None:
                ...
                own_annotations = {}
            msg = "TypedDict('Name', {f0: t0, f1: t1, # CODE_TRUNCATED}); each t must be a type"
            if _TAKES_MODULE:
    ...
    def TypedDict(self, args):
        ...
        if not isinstance(args, dict):
            raise TypeError(
                "TypedDict[# CODE_TRUNCATED] should be used with a single dict argument"
            )
...
if hasattr(typing, "ReadOnly"):  # 3.13+
    ...
    def get_type_hints(obj, globalns=None, localns=None, include_extras=False):
        ...
        'Annotated[T, # CODE_TRUNCATED]', 'Required[T]' or 'NotRequired[T]' with 'T'
    ...
    def _could_be_inserted_optional(t):
        """detects Union[# CODE_TRUNCATED, None] pattern"""
    ...
    def _clean_optional(obj, hints, globalns=None, localns=None):
        # reverts injected Union[# CODE_TRUNCATED, None] cases from typing.get_type_hints
        ...
        for name, value in hints.items():
            # Not a Union[# CODE_TRUNCATED, None] or replacement conditions not fullfilled
...
# 3.9
    ...
    def TypeAlias(self, parameters):
        ...
        For example::
            ...
            Predicate: TypeAlias = Callable[# CODE_TRUNCATED, bool]
...
if not hasattr(typing, 'Concatenate'):
    ...
    class _ConcatenateGenericAlias(list):
        ...
        def copy_with(self, params):
            ...
            elif (not (params[-1] is # CODE_TRUNCATED or isinstance(params[-1], ParamSpec))):
        ...
        def __getitem__(self, args):
            if self.__origin__ in (Generic, Protocol):
                # Can't subscript Generic[# CODE_TRUNCATED] or Protocol[# CODE_TRUNCATED].
...
# 3.9.2
class _EllipsisDummy: # CODE_TRUNCATED


# <=3.10
def _create_concatenate_alias(origin, parameters):
    if parameters[-1] is # CODE_TRUNCATED and sys.version_info < (3, 9, 2):
    ...
    if parameters[-1] is not _EllipsisDummy:
        return concatenate
    # Remove dummy again
    concatenate.__args__ = tuple(p if p is not _EllipsisDummy else # CODE_TRUNCATED
...
def _concatenate_getitem(self, parameters):
    ...
    if not isinstance(parameters, tuple):
        parameters = (parameters,)
    if not (parameters[-1] is # CODE_TRUNCATED or isinstance(parameters[-1], ParamSpec)):
        raise TypeError("The last parameter to Concatenate should be a "
                        "ParamSpec variable or ellipsis.")
    msg = "Concatenate[arg, # CODE_TRUNCATED]: each arg must be a type."
...
# 3.9
    ...
    def TypeGuard(self, parameters):
        ...
        as a type guard.  Such a function should use ``TypeGuard[# CODE_TRUNCATED]`` as its
...
# <=3.12
    ...
    def TypeIs(self, parameters):
        ...
        Sometimes it would be convenient to use a user-defined boolean function
        as a type guard.  Such a function should use ``TypeIs[# CODE_TRUNCATED]`` as its
        return type to alert static type checkers to this intention.
...
# <=3.13
    ...
    def TypeForm(self, parameters):
        ...
        Usage:
            ...
            def cast[T](typ: TypeForm[T], value: Any) -> T: # CODE_TRUNCATED
...
if hasattr(typing, "LiteralString"):  # 3.11+
    ...
    def LiteralString(self, params):
        ...
        Example::
          ...
          def query(sql: LiteralString) -> # CODE_TRUNCATED:
              # CODE_TRUNCATED
...
example:
  ...
  #  `Generic[]`.)
  class Bar(Generic[Unpack[Ts]]): # CODE_TRUNCATED
  Bar[int]  # Valid
...
From Python 3.11, this can also be done using the `*` operator:
    ...
    Foo[*tuple[int, str]]
    class Bar(Generic[*Ts]): # CODE_TRUNCATED
...
`**kwargs` in a function signature. For instance:
  ...
  # *year* of type `int`.
  def foo(**kwargs: Unpack[Movie]): # CODE_TRUNCATED
        ...
        def __typing_unpacked_tuple_args__(self):
            ...
            if isinstance(arg, (typing._GenericAlias, _types.GenericAlias)):
                if arg.__origin__ is not tuple:
                    raise TypeError("Unpack[# CODE_TRUNCATED] must be used with a tuple type")
                return arg.__args__
            return None
...
def _unpack_args(*args):
    ...
    for arg in args:
        ...
        if subargs is not None and (not (subargs and subargs[-1] is # CODE_TRUNCATED)):
...
if _PEP_696_IMPLEMENTED:
        ...
        def __new__(cls, name, *, default=NoDefault):
            ...
            def _typevartuple_prepare_subst(alias, args):
                ...
                for k, arg in enumerate(args):
                    if not isinstance(arg, type):
                        subargs = getattr(arg, '__typing_unpacked_tuple_args__', None)
                        if subargs and len(subargs) == 2 and subargs[-1] is # CODE_TRUNCATED:
    ...
    class TypeVarTuple(_DefaultMixin):
        ...
        Consider the following example::
            ...
            class Array(Generic[*Ts]): # CODE_TRUNCATED

        The ``Ts`` type variable tuple here behaves like ``tuple[T1, T2]``,
        where ``T1`` and ``T2`` are type variables. To use these type variables
        as type parameters of ``Array``, we must *unpack* the type variable tuple using
        the star operator: ``*Ts``. The signature of ``Array`` then behaves
        as if we had simply written ``class Array(Generic[T1, T2]): # CODE_TRUNCATED``.
        ...
        signatures and variable annotations::
            ...
            z = x + x   #        # CODE_TRUNCATED    is Array[Height, Width]
            x.get_shape()  #     # CODE_TRUNCATED    is tuple[Height, Width]
...
if hasattr(typing, "assert_never"):  # 3.11+
    ...
    def assert_never(arg: Never, /) -> Never:
        ...
        if len(value) > _ASSERT_NEVER_REPR_MAX_LENGTH:
            value = value[:_ASSERT_NEVER_REPR_MAX_LENGTH] + '# CODE_TRUNCATED'
    ...
    def dataclass_transform(
        *,
        ...
        field_specifiers: typing.Tuple[
            typing.Union[typing.Type[typing.Any], typing.Callable[# CODE_TRUNCATED, typing.Any]],
            # CODE_TRUNCATED
        ] = (),
        **kwargs: typing.Any,
    ) -> typing.Callable[[T], T]:
        ...
        Example:
            ...
            @dataclass_transform()
            class ModelBase: # CODE_TRUNCATED
            ...
            # Used on a metaclass
            @dataclass_transform()
            class ModelMeta(type): # CODE_TRUNCATED

            class ModelBase(metaclass=ModelMeta): # CODE_TRUNCATED
...
if hasattr(typing, "override"):  # 3.12+
    ...
    _F = typing.TypeVar("_F", bound=typing.Callable[# CODE_TRUNCATED, typing.Any])
...
if sys.version_info >= (3, 13, 3):
    ...
    class deprecated:
        ...
        Usage:
            ...
            def g(x: int) -> int: # CODE_TRUNCATED
            @overload
            def g(x: str) -> int: # CODE_TRUNCATED
...
if sys.version_info < (3, 10):
    def _is_param_expr(arg):
        return arg is # CODE_TRUNCATED or isinstance(
...
if hasattr(_types, "get_original_bases"):
    ...
    def get_original_bases(cls, /):
        ...
        Examples::
            ...
            T = TypeVar("T")
            class Foo(Generic[T]): # CODE_TRUNCATED
            class Bar(Foo[int], float): # CODE_TRUNCATED
            class Baz(list[str]): # CODE_TRUNCATED
    ...
    class TypeAliasType:
        ...
        if sys.version_info < (3, 11):
            def _check_single_param(self, param, recursion=0):
                # Allow [], [int], [int, str], [int, # CODE_TRUNCATED], [int, T]
                if param is # CODE_TRUNCATED:
...
if hasattr(typing, "is_protocol"):
    ...
    def is_protocol(tp: type, /) -> bool:
        ...
        Example::
            ...
            >>> class P(Protocol):
            # CODE_TRUNCATED     def a(self) -> str: # CODE_TRUNCATED
            # CODE_TRUNCATED     b: int
            >>> is_protocol(P)
            True
            >>> is_protocol(int)
            False
...
if hasattr(typing, "Doc"):
    ...
    class Doc:
        ...
        Example::
            ...
            >>> def hi(to: Annotated[str, Doc("Who to say hi to")]) -> None: # CODE_TRUNCATED
        """
    ...
    def get_annotations(obj, *, globals=None, locals=None, eval_str=False,
        ...
        * STRING: return annotations as strings, in a format close to the original
        ...
        if isinstance(obj, type):
            ...
            # "class funclike" from Lib/test/test_inspect# CODE_TRUNCATED on and on it goes.
            ann = getattr(obj, '__annotations__', None)
