...
def _get_signature(call: Callable[# CODE_TRUNCATED, Any]) -> inspect.Signature:
...
def get_typed_signature(call: Callable[# CODE_TRUNCATED, Any]) -> inspect.Signature:
...
def get_typed_return_annotation(call: Callable[# CODE_TRUNCATED, Any]) -> Any:
    signature = _get_signature(call)
    unwrapped = inspect.unwrap(call)
...
def get_dependant(
    *,
    path: str,
    call: Callable[# CODE_TRUNCATED, Any],
...
async def solve_dependencies(
    *,
    ...
    if dependency_cache is None:
        dependency_cache = {}
    for sub_dependant in dependant.dependencies:
        sub_dependant.call = cast(Callable[# CODE_TRUNCATED, Any], sub_dependant.call)
...
def _validate_value_with_model_field(
    *, field: ModelField, value: Any, values: dict[str, Any], loc: tuple[str, # CODE_TRUNCATED]
) -> tuple[Any, list[Any]]:
    if value is None:
        if field.required:
...
def request_params_to_args(
    ...
    if single_not_embedded_field:
        ...
        loc: tuple[str, # CODE_TRUNCATED] = (field_info.in_.value,)
...
async def request_body_to_args(
    ...
    if single_not_embedded_field:
        loc: tuple[str, # CODE_TRUNCATED] = ("body",)
        v_, errors_ = _validate_value_with_model_field(
