...
class BaseAPIResponse(Generic[R]):
    ...
    def _parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        # unwrap `Annotated[T, # CODE_TRUNCATED]` -> `T`
...
class APIResponse(BaseAPIResponse[R]):
    ...
    @overload
    def parse(self, *, to: type[_T]) -> _T: # CODE_TRUNCATED

    @overload
    def parse(self) -> R: # CODE_TRUNCATED
...
class AsyncAPIResponse(BaseAPIResponse[R]):
    ...
    @overload
    async def parse(self, *, to: type[_T]) -> _T: # CODE_TRUNCATED

    @overload
    async def parse(self) -> R: # CODE_TRUNCATED
...
class StreamAlreadyConsumed(OpenAIError):
    ...
    ```py
    response = await client.post(# CODE_TRUNCATED)
    async for line in response.iter_lines():
        # CODE_TRUNCATED  # do something with `line`
...
def extract_response_type(typ: type[BaseAPIResponse[Any]]) -> type:
    ...
    return extract_type_var_from_base(
        typ,
        generic_bases=cast("tuple[type, # CODE_TRUNCATED]", (BaseAPIResponse, APIResponse, AsyncAPIResponse)),
        index=0,
    )
