...
class BaseModel(pydantic.BaseModel):
    ...
    if TYPE_CHECKING:
        ...
        ```py
        completion = await client.chat.completions.create(# CODE_TRUNCATED)
...
def construct_type(*, value: object, type_: object, metadata: Optional[List[Any]] = None) -> object:
    ...
    if metadata is not None and len(metadata) > 0:
        meta: tuple[Any, # CODE_TRUNCATED] = tuple(metadata)
...
def _build_discriminated_union_meta(*, union: type, meta_annotations: tuple[Any, # CODE_TRUNCATED]) -> DiscriminatorDetails | None:
