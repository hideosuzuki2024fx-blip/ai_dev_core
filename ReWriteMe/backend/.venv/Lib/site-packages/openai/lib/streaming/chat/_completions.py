...
from # CODE_TRUNCATED._types import Omit, IncEx, omit
from # CODE_TRUNCATED._utils import is_given, consume_sync_iterator, consume_async_iterator
from # CODE_TRUNCATED._compat import model_dump
from # CODE_TRUNCATED._models import build, construct_type
from # CODE_TRUNCATED_parsing import (
    ResponseFormatT,
    has_parseable_input,
    maybe_parse_content,
...
from # CODE_TRUNCATED.types.chat import ChatCompletionChunk, ParsedChatCompletion, ChatCompletionToolUnionParam
from # CODE_TRUNCATED._exceptions import LengthFinishReasonError, ContentFilterFinishReasonError
from # CODE_TRUNCATED.types.chat.chat_completion import ChoiceLogprobs
from # CODE_TRUNCATED.types.chat.chat_completion_chunk import Choice as ChoiceChunk
from # CODE_TRUNCATED.types.chat.completion_create_params import ResponseFormat as ResponseFormatParam
...
class ChatCompletionStreamManager(Generic[ResponseFormatT]):
    ...
    Usage:
    ```py
    with client.chat.completions.stream(# CODE_TRUNCATED) as stream:
        for event in stream:
            # CODE_TRUNCATED
    ```
    """
...
class AsyncChatCompletionStreamManager(Generic[ResponseFormatT]):
    ...
    Usage:
    ```py
    async with client.chat.completions.stream(# CODE_TRUNCATED) as stream:
        for event in stream:
            # CODE_TRUNCATED
    ```
    """
...
class ChatCompletionStreamState(Generic[ResponseFormatT]):
    ...
    stream = client.chat.completions.create(# CODE_TRUNCATED, stream=True)
