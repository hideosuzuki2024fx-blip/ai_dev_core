    ...
    if os.path.isfile(fname):
        try:
            if fname.lower().endswith(".csv") or fname.lower().endswith(".tsv"):
                ...
                if len(sheets) > 1:
                    immediate_msg += "\n- Your Excel file contains more than one sheet. Please either save as csv or ensure all data is present in the first sheet. WARNING: Reading only the first sheet# CODE_TRUNCATED"
...
def write_out_file(df: pd.DataFrame, fname: str, any_remediations: bool, auto_accept: bool) -> None:
    ...
    if not any_remediations and not split:
        ...
        sys.stdout.write("Aborting# CODE_TRUNCATED did not write the file\n")
...
def apply_validators(
    ...
    auto_accept: bool,
    write_out_file_func: Callable[# CODE_TRUNCATED, Any],
) -> None:
