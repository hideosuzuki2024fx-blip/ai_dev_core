...
if TYPE_CHECKING:  # pragma: no cover
    ...
    class SupportsRead(Protocol):
        def read(self, __n: int) -> bytes: # CODE_TRUNCATED
    ...
    class _FormProtocol(Protocol):
        def write(self, data: bytes) -> int: # CODE_TRUNCATED

        def finalize(self) -> None: # CODE_TRUNCATED

        def close(self) -> None: # CODE_TRUNCATED

    class FieldProtocol(_FormProtocol, Protocol):
        def __init__(self, name: bytes | None) -> None: # CODE_TRUNCATED

        def set_none(self) -> None: # CODE_TRUNCATED

    class FileProtocol(_FormProtocol, Protocol):
        def __init__(self, file_name: bytes | None, field_name: bytes | None, config: FileConfig) -> None: # CODE_TRUNCATED
...
class Field:
    ...
    def __repr__(self) -> str:
        if self.value is not None and len(self.value) > 97:
            ...
            # quote.
            v = repr(self.value[:97])[:-1] + "# CODE_TRUNCATED'"
        else:
            v = repr(self.value)
...
class File:
    ...
    def _get_disk_file(self) -> BufferedRandom:
        ...
        # If we have a directory and are to keep the filename# CODE_TRUNCATED
...
class BaseParser:
    ...
    def callback(
        ...
        if func is None:
            return
        func = cast("Callable[# CODE_TRUNCATED, Any]", func)
        # Depending on whether we're given a buffer# CODE_TRUNCATED
        if data is not None:
    ...
    def set_callback(self, name: CallbackName, new_func: Callable[# CODE_TRUNCATED, Any] | None) -> None:
...
class QuerystringParser(BaseParser):
    ...
    Args:
        ...
        strict_parsing: Whether or not to parse the body strictly.  Defaults to False.  If this is set to True, then the
            ...
            (e.g. "foo=bar", or "foo="), it is always included.  If a field has no equals sign (e.g. "# CODE_TRUNCATED&name&# CODE_TRUNCATED"),
    ...
    def _internal_write(self, data: bytes, length: int) -> int:
        ...
        while i < length:
            ...
            # Depending on our state# CODE_TRUNCATED
    ...
    def _internal_write(self, data: bytes, length: int) -> int:
        ...
        while i < length:
            ...
            if state == MultipartState.START:
                ...
                # the end of the boundary# CODE_TRUNCATED
                if index < boundary_length:
                    # If the character matches# CODE_TRUNCATED
                    if boundary[index] == c:
                ...
                # a LF.
                    ...
                    # character already)# CODE_TRUNCATED
                    ...
                    # seen a hyphen already)# CODE_TRUNCATED
