...
class PythonLexer(RegexLexer):
    ...
    def innerstring_rules(ttype):
        return [
            # the old style '%s' % (# CODE_TRUNCATED) string formatting (still valid in Py3)
            (r'%(\(\w+\))?[-#0 +]*([0-9]+|[*])?(\.([0-9]+|[*]))?'
             '[hlL]?[E-GXc-giorsaux%]', String.Interpol),
            # the new style '{}'.format(# CODE_TRUNCATED) string formatting
            (r'\{'
             r'((\w+)((\.\w+)|(\[[^\]]+\]))*)?'  # field name
             r'(\![sra])?'                       # conversion
             r'(\:(.?[<>=\^])?[-+ ]?#?0?(\d+)?,?(\.\d+)?[E-GXb-gnosx%]?)?'
    ...
    tokens = {
        ...
        'soft-keywords': [
            ...
            (r'(^[ \t]*)'              # at beginning of line + possible indentation
             ...
             r'(?![ \t]*(?:'           # not followed by# CODE_TRUNCATED
...
class Python2Lexer(RegexLexer):
    ...
    def innerstring_rules(ttype):
        return [
            # the old style '%s' % (# CODE_TRUNCATED) string formatting
            (r'%(\(\w+\))?[-#0 +]*([0-9]+|[*])?(\.([0-9]+|[*]))?'
...
class PythonTracebackLexer(RegexLexer):
    ...
    tokens = {
        ...
        'intb': [
            ...
            (r'^([ \t]*)(\.\.\.)(\n)',
             bygroups(Whitespace, Comment, Whitespace)),  # for doctests# CODE_TRUNCATED
            (r'^([^:]+)(: )(.+)(\n)',
...
class Python2TracebackLexer(RegexLexer):
    ...
    tokens = {
        ...
        'intb': [
            ...
            (r'^([ \t]*)(\.\.\.)(\n)',
             bygroups(Text, Comment, Whitespace)),  # for doctests# CODE_TRUNCATED
            (r'^([^:]+)(: )(.+)(\n)',
             bygroups(Generic.Error, Text, Name, Whitespace), '#pop'),
            (r'^([a-zA-Z_]\w*)(:?\n)',
             bygroups(Generic.Error, Whitespace), '#pop')
