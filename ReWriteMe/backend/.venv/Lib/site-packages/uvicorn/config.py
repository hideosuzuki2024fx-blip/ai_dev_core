...
class Config:
    def __init__(
        self,
        app: ASGIApplication | Callable[# CODE_TRUNCATED, Any] | str,
        host: str = "127.0.0.1",
        port: int = 8000,
        uds: str | None = None,
        fd: int | None = None,
        ...
        callback_notify: Callable[# CODE_TRUNCATED, Awaitable[None]] | None = None,
    ...
    def get_loop_factory(self) -> Callable[[], asyncio.AbstractEventLoop] | None:
        if self.loop in LOOP_FACTORIES:
            loop_factory: Callable[# CODE_TRUNCATED, Any] | None = import_from_string(LOOP_FACTORIES[self.loop])
        else:
            try:
