...
class AsyncLRUCacheWrapper(Generic[P, T]):
    def __init__(
        self,
        func: Callable[# CODE_TRUNCATED, Awaitable[T]],
        maxsize: int | None,
        typed: bool,
        always_checkpoint: bool,
    ):
    ...
    async def __call__(self, *args: P.args, **kwargs: P.kwargs) -> T:
        ...
        key: tuple[Any, # CODE_TRUNCATED] = args
        if kwargs:
...
class _LRUCacheWrapper(Generic[T]):
    ...
    ) -> AsyncLRUCacheWrapper[P, T]: # CODE_TRUNCATED

    @overload
    def __call__(
        self, func: Callable[# CODE_TRUNCATED, T], /
    ) -> functools._lru_cache_wrapper[T]: # CODE_TRUNCATED

    def __call__(
        self, f: Callable[P, Coroutine[Any, Any, T]] | Callable[# CODE_TRUNCATED, T], /
...
@overload
def cache(func: Callable[# CODE_TRUNCATED, T], /) -> functools._lru_cache_wrapper[T]: # CODE_TRUNCATED


def cache(
    func: Callable[# CODE_TRUNCATED, T] | Callable[P, Coroutine[Any, Any, T]], /
) -> AsyncLRUCacheWrapper[P, T] | functools._lru_cache_wrapper[T]:
    """
...
@overload
def lru_cache(
    *, maxsize: int | None = # CODE_TRUNCATED, typed: bool = # CODE_TRUNCATED, always_checkpoint: bool = # CODE_TRUNCATED
) -> _LRUCacheWrapper[Any]: # CODE_TRUNCATED
...
@overload
def lru_cache(func: Callable[# CODE_TRUNCATED, T], /) -> functools._lru_cache_wrapper[T]: # CODE_TRUNCATED


def lru_cache(
    func: Callable[P, Coroutine[Any, Any, T]] | Callable[# CODE_TRUNCATED, T] | None = None,
    /,
    *,
