...
class PydanticModelTransformer:
    ...
    def adjust_decorator_signatures(self) -> None:
        """When we decorate a function `f` with `pydantic.validator(# CODE_TRUNCATED)`, `pydantic.field_validator`
        or `pydantic.serializer(# CODE_TRUNCATED)`, mypy sees `f` as a regular method taking a `self` instance,
    ...
    def collect_config(self) -> ModelConfigData:  # noqa: C901 (ignore complexity)
        ...
        # Handle `class MyModel(BaseModel, <name>=<expr>, # CODE_TRUNCATED):`
    ...
    def collect_field_or_class_var_from_stmt(  # noqa C901
        ...
        if sym.type is None and node.is_final and node.is_inferred:
            ...
            # argument for Final[# CODE_TRUNCATED].
            typ = self._api.analyze_simple_literal_type(stmt.rvalue, is_final=True)
            if typ:
                node.type = typ
            else:
                self._api.fail(
                    'Need type argument for Final[# CODE_TRUNCATED] with non-literal default in BaseModel',
                    stmt,
                )
        ...
        if name == 'extra':
            if isinstance(arg, StrExpr):
                ...
                if not lax_extra:
                    ...
                    # type. Note that you can still get proper type checking via: `model_config = ConfigDict(# CODE_TRUNCATED)`, just
    ...
    def get_has_default(stmt: AssignmentStmt) -> bool:
        ...
        if isinstance(expr, CallExpr) and isinstance(expr.callee, RefExpr) and expr.callee.fullname == FIELD_FULLNAME:
            ...
            # * there is a positional argument that is not `# CODE_TRUNCATED`
            # * there is a keyword argument named "default" that is not `# CODE_TRUNCATED`
            ...
            return False
        # Has no default if the "default value" is Ellipsis (i.e., `field_name: Annotation = # CODE_TRUNCATED`)
