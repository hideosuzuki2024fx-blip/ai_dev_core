...
class GenerateJsonSchema:
    ...
    def tuple_schema(self, schema: core_schema.TupleSchema) -> JsonSchemaValue:
        ...
        str, bool]` or `tuple[int, # CODE_TRUNCATED]`.
    ...
    def _update_class_schema(self, json_schema: JsonSchemaValue, cls: type[Any], config: ConfigDict) -> None:
        ...
        * deprecated
        ...
        if isinstance(json_schema_extra, dict):
            ...
            # FIXME: why are there type ignores here? We support two signatures for json_schema_extra callables# CODE_TRUNCATED
...
_HashableJsonValue: TypeAlias = Union[
    int, float, str, bool, None, tuple['_HashableJsonValue', # CODE_TRUNCATED], tuple[tuple[str, '_HashableJsonValue'], # CODE_TRUNCATED]
]
...
class Examples:
    ...
    @overload
    def __init__(self, examples: list[Any], mode: Literal['validation', 'serialization'] | None = None) -> None: # CODE_TRUNCATED
        ...
        if isinstance(current, dict):
            for key, value in current.items():
                if key == 'examples' and isinstance(value, list):
                    # Skip examples that may contain arbitrary values and references
                    # (e.g. `{"examples": [{"$ref": "# CODE_TRUNCATED"}]}`). Note: checking for value
...
if TYPE_CHECKING:
    SkipJsonSchema = Annotated[AnyType, # CODE_TRUNCATED]
else:
