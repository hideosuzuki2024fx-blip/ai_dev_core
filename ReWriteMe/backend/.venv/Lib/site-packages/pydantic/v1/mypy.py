...
def parse_mypy_version(version: str) -> Tuple[int, # CODE_TRUNCATED]:
...
class PydanticModelTransformer:
    ...
    def adjust_validator_signatures(self) -> None:
        """When we decorate a function `f` with `pydantic.validator(# CODE_TRUNCATED), mypy sees
    ...
    def get_is_required(cls: ClassDef, stmt: AssignmentStmt, lhs: NameExpr) -> bool:
        ...
        if isinstance(expr, CallExpr) and isinstance(expr.callee, RefExpr) and expr.callee.fullname == FIELD_FULLNAME:
            ...
            # only required if default is Ellipsis (i.e., `field_name: Annotation = Field(# CODE_TRUNCATED)`) or if default_factory
            # is specified.
            ...
            value_type = get_proper_type(cls.info[lhs.name].type)
            return not PydanticModelTransformer.type_has_implicit_default(value_type)
        # Only required if the "default value" is Ellipsis (i.e., `field_name: Annotation = # CODE_TRUNCATED`)
