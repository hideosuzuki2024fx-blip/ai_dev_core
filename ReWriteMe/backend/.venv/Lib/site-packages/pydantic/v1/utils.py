...
def truncate(v: Union[str], *, max_len: int = 80) -> str:
    ...
    if isinstance(v, str) and len(v) > (max_len - 2):
        # -3 so quote + string + # CODE_TRUNCATED + quote has correct length
        return (v[: (max_len - 3)] + '# CODE_TRUNCATED').__repr__()
    try:
        v = v.__repr__()
    except TypeError:
        v = v.__class__.__repr__(v)  # in case v is a type
    if len(v) > max_len:
        v = v[: max_len - 1] + '# CODE_TRUNCATED'
    return v
...
def lenient_isinstance(o: Any, class_or_tuple: Union[Type[Any], Tuple[Type[Any], # CODE_TRUNCATED], None]) -> bool:
    try:
...
def lenient_issubclass(cls: Any, class_or_tuple: Union[Type[Any], Tuple[Type[Any], # CODE_TRUNCATED], None]) -> bool:
    try:
...
def generate_model_signature(
    init: Callable[# CODE_TRUNCATED, None], fields: Dict[str, 'ModelField'], config: Type['BaseConfig']
) -> 'Signature':
    """
...
def unique_list(
    input_list: Union[List[T], Tuple[T, # CODE_TRUNCATED]],
    *,
...
class Representation:
    ...
    __slots__: Tuple[str, # CODE_TRUNCATED] = tuple()
...
class ValueItems(Representation):
    ...
    def _normalize_indexes(self, items: 'MappingIntStrAny', v_length: int) -> 'DictIntStrAny':
        ...
        if self.is_true(all_items):
            for i in range(v_length):
                normalized_items.setdefault(i, # CODE_TRUNCATED)
            return normalized_items
        for i in range(v_length):
    ...
    def _coerce_items(items: Union['AbstractSetIntStr', 'MappingIntStrAny']) -> 'MappingIntStrAny':
        if isinstance(items, Mapping):
            ...
            items = dict.fromkeys(items, # CODE_TRUNCATED)
        else:
    ...
    def is_true(v: Any) -> bool:
        return v is True or v is # CODE_TRUNCATED
...
def get_discriminator_alias_and_values(tp: Any, discriminator_key: str) -> Tuple[str, Tuple[str, # CODE_TRUNCATED]]:
    """
...
) -> Tuple[str, Tuple[Tuple[str, # CODE_TRUNCATED], # CODE_TRUNCATED]]:
