...
def apply_each_item_validators(
    ...
    if schema['type'] == 'nullable':
        ...
        raise TypeError(
            f'`@validator(# CODE_TRUNCATED, each_item=True)` cannot be applied to fields with a schema of {schema["type"]}'
        )
    return schema
...
class GenerateSchema:
    """Generate core schema for a Pydantic model, dataclass and types like `str`, `datetime`, # CODE_TRUNCATED ."""
    ...
    def _add_js_function(self, metadata_schema: CoreSchema, js_function: Callable[# CODE_TRUNCATED, Any]) -> None:
                    ...
                    for candidate_cls in cls.__mro__[:-1]:
                        ...
                        if extras_annotation is not None:
                            ...
                            if tp not in DICT_TYPES:
                                raise PydanticSchemaGenerationError(
                                    'The type annotation for `__pydantic_extra__` must be `dict[str, # CODE_TRUNCATED]`'
                                )
    ...
    @overload
    def _get_args_resolving_forward_refs(self, obj: Any, required: Literal[True]) -> tuple[Any, # CODE_TRUNCATED]: # CODE_TRUNCATED

    @overload
    def _get_args_resolving_forward_refs(self, obj: Any) -> tuple[Any, # CODE_TRUNCATED] | None: # CODE_TRUNCATED

    def _get_args_resolving_forward_refs(self, obj: Any, required: bool = False) -> tuple[Any, # CODE_TRUNCATED] | None:
        args = get_args(obj)
        if args:
    ...
    def _namedtuple_schema(self, namedtuple_cls: Any, origin: Any) -> core_schema.CoreSchema:
        ...
        with (
            ...
            try:
                ...
                raise PydanticUndefinedAnnotation.from_name_error(e) from e
            if not annotations:
                # annotations is empty, happens if namedtuple_cls defined via collections.namedtuple(# CODE_TRUNCATED)
    ...
    def _tuple_schema(self, tuple_type: Any) -> core_schema.CoreSchema:
        """Generate schema for a Tuple, e.g. `tuple[int, str]` or `tuple[int, # CODE_TRUNCATED]`."""
    ...
    def _union_is_subclass_schema(self, union_type: Any) -> core_schema.CoreSchema:
        """Generate schema for `type[Union[X, # CODE_TRUNCATED]]`."""
        args = self._get_args_resolving_forward_refs(union_type, required=True)
    ...
    def _subclass_schema(self, type_: Any) -> core_schema.CoreSchema:
        ...
        # Assume `type[Annotated[<typ>, # CODE_TRUNCATED]]` is equivalent to `type[<typ>]`:
            ...
            if parameter_mode is not None:
                ...
                if unpack_type is not None:
                    ...
                    if not is_typeddict(origin):
                        raise PydanticUserError(
                            f'Expected a `TypedDict` class inside `Unpack[# CODE_TRUNCATED]`, got {unpack_type!r}',
    ...
    def _annotated_schema(self, annotated_type: Any) -> core_schema.CoreSchema:
        """Generate schema for an Annotated type, e.g. `Annotated[int, Field(# CODE_TRUNCATED)]` or `Annotated[int, Gt(0)]`."""
        FieldInfo = import_cached_field_info()
...
_VALIDATOR_F_MATCH: Mapping[
    ...
    Callable[[Callable[# CODE_TRUNCATED, Any], core_schema.CoreSchema], core_schema.CoreSchema],
] = {
