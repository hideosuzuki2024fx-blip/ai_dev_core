...
def lenient_isinstance(o: Any, class_or_tuple: type[Any] | tuple[type[Any], # CODE_TRUNCATED] | None) -> bool:  # pragma: no cover
    try:
...
def unique_list(
    input_list: list[T] | tuple[T, # CODE_TRUNCATED],
    *,
...
class ValueItems(_repr.Representation):
    ...
    def _coerce_items(items: AbstractSetIntStr | MappingIntStrAny) -> MappingIntStrAny:
        if isinstance(items, Mapping):
            ...
            items = dict.fromkeys(items, # CODE_TRUNCATED)  # type: ignore
        else:
...
if TYPE_CHECKING:
    ...
    def LazyClassAttribute(name: str, get_value: Callable[[], T]) -> T: # CODE_TRUNCATED
...
class deprecated_instance_property(Generic[_ModelT, _RT]):
    ...
    @overload
    def __get__(self, instance: None, objtype: type[_ModelT]) -> _RT: # CODE_TRUNCATED
    @overload
    @deprecated(
    ...
    def __get__(self, instance: _ModelT, objtype: type[_ModelT]) -> _RT: # CODE_TRUNCATED
