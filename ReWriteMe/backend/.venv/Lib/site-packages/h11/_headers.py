...
# important, # CODE_TRUNCATED), there doesn't appear to be any standard way to handle
# headers in Python -- they're almost like dicts, but# CODE_TRUNCATED actually just
# aren't. For now we punt and just use a super simple representation: headers
# are a list of pairs
#
#   [(name1, value1), (name2, value2), # CODE_TRUNCATED]
#
...
def set_comma_header(headers: Headers, name: bytes, new_values: List[bytes]) -> Headers:
    ...
    # here given the cases where we're using `set_comma_header`# CODE_TRUNCATED
    #
