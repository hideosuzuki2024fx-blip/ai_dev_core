...
class DBAPIModule(Protocol):
    class Error(Exception):
        def __getattr__(self, key: str) -> Any: # CODE_TRUNCATED
...
class DBAPIConnection(Protocol):
    ...
    def close(self) -> None: # CODE_TRUNCATED

    def commit(self) -> None: # CODE_TRUNCATED

    def cursor(self, *args: Any, **kwargs: Any) -> DBAPICursor: # CODE_TRUNCATED

    def rollback(self) -> None: # CODE_TRUNCATED

    def __getattr__(self, key: str) -> Any: # CODE_TRUNCATED

    def __setattr__(self, key: str, value: Any) -> None: # CODE_TRUNCATED
...
class DBAPICursor(Protocol):
    ...
    @property
    def rowcount(self) -> int: # CODE_TRUNCATED
    ...
    def fetchone(self) -> Optional[Any]: # CODE_TRUNCATED

    def fetchmany(self, size: int = # CODE_TRUNCATED) -> Sequence[Any]: # CODE_TRUNCATED

    def fetchall(self) -> Sequence[Any]: # CODE_TRUNCATED

    def setinputsizes(self, sizes: Sequence[Any]) -> None: # CODE_TRUNCATED

    def setoutputsize(self, size: Any, column: Any) -> None: # CODE_TRUNCATED

    def callproc(
        self, procname: str, parameters: Sequence[Any] = # CODE_TRUNCATED
    ) -> Any: # CODE_TRUNCATED

    def nextset(self) -> Optional[bool]: # CODE_TRUNCATED
...
VersionInfoType = Tuple[Union[int, str], # CODE_TRUNCATED]
...
class Dialect(EventTarget):
    ...
    server_version_info: Optional[Tuple[Any, # CODE_TRUNCATED]]
    ...
    execute_sequence_format: Union[
        Type[Tuple[Any, # CODE_TRUNCATED]], Type[Tuple[List[Any]]]
    ]
    ...
    "INSERT # CODE_TRUNCATED (col) VALUES (DEFAULT)".

    """

    supports_default_values: bool
    """dialect supports INSERT# CODE_TRUNCATED DEFAULT VALUES syntax"""

    supports_default_metavalue: bool
    """dialect supports INSERT# CODE_TRUNCATED(col) VALUES (DEFAULT) syntax.
    ...
    """Target database supports INSERT# CODE_TRUNCATEDVALUES with multiple value
    sets, i.e. INSERT INTO table (cols) VALUES (# CODE_TRUNCATED), (# CODE_TRUNCATED), (# CODE_TRUNCATED), # CODE_TRUNCATED

    """

    insert_executemany_returning: bool
    """dialect / driver / database supports some means of providing
    INSERT# CODE_TRUNCATEDRETURNING support when dialect.do_executemany() is used.
    ...
    if TYPE_CHECKING:
        ...
        def _overrides_default(self, method_name: str) -> bool: # CODE_TRUNCATED
...
class AdaptedConnection:
    ...
    def run_async(self, fn: Callable[[Any], Awaitable[_T]]) -> _T:
        ...
        E.g.::
            ...
            engine = create_async_engine(# CODE_TRUNCATED)


            @event.listens_for(engine.sync_engine, "connect")
            def register_custom_types(
                dbapi_connection,  # # CODE_TRUNCATED
            ):
                dbapi_connection.run_async(
                    lambda connection: connection.set_type_codec(
                        "MyCustomType", encoder, decoder, # CODE_TRUNCATED
                    )
                )
