...
class _ManagerFactory(Protocol):
    def __call__(self, class_: Type[_O]) -> ClassManager[_O]: # CODE_TRUNCATED
...
class ClassManager(
    ...
    init_method: Optional[Callable[# CODE_TRUNCATED, None]]
    original_init: Optional[Callable[# CODE_TRUNCATED, None]] = None
    ...
    def _update_state(
        ...
        init_method: Optional[Callable[# CODE_TRUNCATED, None]] = None,
    ) -> None:
        if mapper:
            self.mapper = mapper  #
        if registry:
