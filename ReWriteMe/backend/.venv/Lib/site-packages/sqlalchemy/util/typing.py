...
class ArgsTypeProtocol(Protocol):
    ...
    __args__: Tuple[_AnnotationScanType, # CODE_TRUNCATED]
...
class GenericProtocol(Protocol[_T]):
    ...
    # ``__args__``
    #
    # def copy_with(self, params: Tuple[_AnnotationScanType, # CODE_TRUNCATED]) -> Type[_T]:
    #     # CODE_TRUNCATED
...
class SupportsKeysAndGetItem(Protocol[_KT, _VT_co]):
    def keys(self) -> Iterable[_KT]: # CODE_TRUNCATED

    def __getitem__(self, __k: _KT) -> _VT_co: # CODE_TRUNCATED
...
def _copy_generic_annotation_with(
    annotation: GenericProtocol[_T], elements: Tuple[_AnnotationScanType, # CODE_TRUNCATED]
) -> Type[_T]:
...
@overload
def de_optionalize_union_types(type_: str) -> str: # CODE_TRUNCATED


@overload
def de_optionalize_union_types(type_: Type[Any]) -> Type[Any]: # CODE_TRUNCATED


@overload
def de_optionalize_union_types(
    type_: _AnnotationScanType,
) -> _AnnotationScanType: # CODE_TRUNCATED
...
def _de_optionalize_fwd_ref_union_types(
    ...
    """return the non-optional type for Optional[], Union[None, # CODE_TRUNCATED], x|None,
...
def is_origin_of_cls(
    type_: Any, class_obj: Union[Tuple[Type[Any], # CODE_TRUNCATED], Type[Any]]
) -> bool:
...
class DescriptorProto(Protocol):
    def __get__(self, instance: object, owner: Any) -> Any: # CODE_TRUNCATED
...
class DescriptorReference(Generic[_DESC]):
    ...
    if TYPE_CHECKING:
        ...
        def __get__(self, instance: object, owner: Any) -> _DESC: # CODE_TRUNCATED

        def __set__(self, instance: Any, value: _DESC) -> None: # CODE_TRUNCATED
...
class RODescriptorReference(Generic[_DESC_co]):
    ...
    if TYPE_CHECKING:
        ...
        def __get__(self, instance: object, owner: Any) -> _DESC_co: # CODE_TRUNCATED

        def __set__(self, instance: Any, value: Any) -> NoReturn: # CODE_TRUNCATED

        def __delete__(self, instance: Any) -> NoReturn: # CODE_TRUNCATED


_FN = TypeVar("_FN", bound=Optional[Callable[# CODE_TRUNCATED, Any]])
...
class CallableReference(Generic[_FN]):
    ...
    if TYPE_CHECKING:
        ...
        def __get__(self, instance: object, owner: Any) -> _FN: # CODE_TRUNCATED

        def __set__(self, instance: Any, value: _FN) -> None: # CODE_TRUNCATED

        def __delete__(self, instance: Any) -> None: # CODE_TRUNCATED


class _TypingInstances:
    def __getattr__(self, key: str) -> tuple[type, # CODE_TRUNCATED]:
        types = tuple(
            {
                t
                for t in [
