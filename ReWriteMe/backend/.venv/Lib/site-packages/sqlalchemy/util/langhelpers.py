...
_F = TypeVar("_F", bound=Callable[# CODE_TRUNCATED, Any])
...
def method_is_overridden(
    ...
    against_method: Callable[# CODE_TRUNCATED, Any],
) -> bool:
...
def decode_slice(slc: slice) -> Tuple[Any, # CODE_TRUNCATED]:
...
_Fn = TypeVar("_Fn", bound="Callable[# CODE_TRUNCATED, Any]")

# this seems to be in flux in recent mypy versions


def decorator(target: Callable[# CODE_TRUNCATED, Any]) -> Callable[[_Fn], _Fn]:
    ...
    def decorate(fn: _Fn) -> _Fn:
        ...
        names = (
            tuple(cast("Tuple[str, # CODE_TRUNCATED]", spec[0]))
            + cast("Tuple[str, # CODE_TRUNCATED]", spec[1:3])
            + (fn.__name__,)
        )
...
class PluginLoader:
    def __init__(
        self, group: str, auto_fn: Optional[Callable[# CODE_TRUNCATED, Any]] = None
    ):
        self.group = group
...
def get_cls_kwargs(
    ...
    *,
    ...
    raiseerr: Literal[True] = # CODE_TRUNCATED,
) -> Set[str]: # CODE_TRUNCATED


@overload
def get_cls_kwargs(
    cls: type, *, _set: Optional[Set[str]] = None, raiseerr: bool = False
) -> Optional[Set[str]]: # CODE_TRUNCATED
...
def get_func_kwargs(func: Callable[# CODE_TRUNCATED, Any]) -> List[str]:
...
def get_callable_argspec(
    fn: Callable[# CODE_TRUNCATED, Any], no_self: bool = False, _is_init: bool = False
) -> compat.FullArgSpec:
...
def format_argspec_plus(
    fn: Union[Callable[# CODE_TRUNCATED, Any], compat.FullArgSpec], grouped: bool = True
...
class generic_fn_descriptor(Generic[_T_co]):
    ...
    fget: Callable[# CODE_TRUNCATED, _T_co]
    __doc__: Optional[str]
    __name__: str

    def __init__(self, fget: Callable[# CODE_TRUNCATED, _T_co], doc: Optional[str] = None):
        self.fget = fget
        self.__doc__ = doc or fget.__doc__
        self.__name__ = fget.__name__

    @overload
    def __get__(self: _GFD, obj: None, cls: Any) -> _GFD: # CODE_TRUNCATED

    @overload
    def __get__(self, obj: object, cls: Any) -> _T_co: # CODE_TRUNCATED
    ...
    if TYPE_CHECKING:
        ...
        def __set__(self, instance: Any, value: Any) -> None: # CODE_TRUNCATED

        def __delete__(self, instance: Any) -> None: # CODE_TRUNCATED
...
class HasMemoized:
    ...
    class memoized_attribute(memoized_property[_T]):
        ...
        fget: Callable[# CODE_TRUNCATED, _T]
        __doc__: Optional[str]
        __name__: str

        def __init__(self, fget: Callable[# CODE_TRUNCATED, _T], doc: Optional[str] = None):
            self.fget = fget
            self.__doc__ = doc or fget.__doc__
            self.__name__ = fget.__name__

        @overload
        def __get__(self: _MA, obj: None, cls: Any) -> _MA: # CODE_TRUNCATED

        @overload
        def __get__(self, obj: Any, cls: Any) -> _T: # CODE_TRUNCATED
...
def constructor_key(obj: Any, cls: Type[Any]) -> Tuple[Any, # CODE_TRUNCATED]:
...
def counter() -> Callable[[], int]:
    ...
    # avoid the 2to3 "next" transformation# CODE_TRUNCATED
    def _next():
        with lock:
            return next(counter)
...
def assert_arg_type(
    arg: Any, argtype: Union[Tuple[Type[Any], # CODE_TRUNCATED], Type[Any]], name: str
) -> Any:
...
class hybridproperty(Generic[_T]):
    def __init__(self, func: Callable[# CODE_TRUNCATED, _T]):
        self.func = func
        self.clslevel = func
    ...
    def classlevel(self, func: Callable[# CODE_TRUNCATED, Any]) -> hybridproperty[_T]:
        self.clslevel = func
        return self
...
class rw_hybridproperty(Generic[_T]):
    def __init__(self, func: Callable[# CODE_TRUNCATED, _T]):
        self.func = func
        self.clslevel = func
        self.setfn: Optional[Callable[# CODE_TRUNCATED, Any]] = None
    ...
    def setter(self, func: Callable[# CODE_TRUNCATED, Any]) -> rw_hybridproperty[_T]:
        self.setfn = func
        return self

    def classlevel(self, func: Callable[# CODE_TRUNCATED, Any]) -> rw_hybridproperty[_T]:
        self.clslevel = func
        return self
...
class hybridmethod(Generic[_T]):
    ...
    def __get__(self, instance: Any, owner: Any) -> Callable[# CODE_TRUNCATED, _T]:
        if instance is None:
            return self.clslevel.__get__(owner, owner.__class__)  # type:ignore
        else:
            return self.func.__get__(instance, owner)  # type:ignore

    def classlevel(self, func: Callable[# CODE_TRUNCATED, Any]) -> hybridmethod[_T]:
        self.clslevel = func
        return self
...
class _IntFlagMeta(type):
    def __init__(
        cls,
        classname: str,
        bases: Tuple[Type[Any], # CODE_TRUNCATED],
        dict_: Dict[str, Any],
        **kw: Any,
    ) -> None:
        items: List[symbol]
...
def warn_exception(func: Callable[# CODE_TRUNCATED, Any], *args: Any, **kwargs: Any) -> Any:
...
def ellipses_string(value, len_=25):
    try:
        if len(value) > len_:
            return "%s# CODE_TRUNCATED" % value[0:len_]
        else:
            return value
    except TypeError:
        return value
...
def only_once(
    fn: Callable[# CODE_TRUNCATED, _T], retry_on_exception: bool
) -> Callable[# CODE_TRUNCATED, Optional[_T]]:
...
class EnsureKWArg:
    ...
    @classmethod
    def _wrap_w_kw(cls, fn: Callable[# CODE_TRUNCATED, Any]) -> Callable[# CODE_TRUNCATED, Any]:
...
def repr_tuple_names(names: List[str]) -> Optional[str]:
    ...
    if flag:
        ...
        return "%s, # CODE_TRUNCATED, %s" % (", ".join(res[0:3]), res[-1])
