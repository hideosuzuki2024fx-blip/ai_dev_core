...
class Properties(Generic[_T]):
    ...
    @overload
    def get(self, key: str) -> Optional[_T]: # CODE_TRUNCATED

    @overload
    def get(self, key: str, default: Union[_DT, _T]) -> Union[_DT, _T]: # CODE_TRUNCATED
...
class LRUCache(typing.MutableMapping[_KT, _VT]):
    ...
    def __init__(
        ...
        capacity: int = 100,
        threshold: float = 0.5,
        size_alert: Optional[Callable[# CODE_TRUNCATED, None]] = None,
    ):
    ...
    @overload
    def get(self, key: _KT) -> Optional[_VT]: # CODE_TRUNCATED

    @overload
    def get(self, key: _KT, default: Union[_VT, _T]) -> Union[_VT, _T]: # CODE_TRUNCATED
...
class _CreateFuncType(Protocol[_T_co]):
    def __call__(self) -> _T_co: # CODE_TRUNCATED


class _ScopeFuncType(Protocol):
    def __call__(self) -> Any: # CODE_TRUNCATED
