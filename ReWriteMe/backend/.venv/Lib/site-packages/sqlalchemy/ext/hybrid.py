...
.. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy import select
    >>> print(
    # CODE_TRUNCATED     select(User, User.balance)
    # CODE_TRUNCATED     .join(User.accounts)
    # CODE_TRUNCATED     .filter(User.balance > 5000)
    # CODE_TRUNCATED )
...
.. sourcecode:: pycon+sql
    ...
    >>> print(
    # CODE_TRUNCATED     select(User, User.balance)
    # CODE_TRUNCATED     .outerjoin(User.accounts)
    # CODE_TRUNCATED     .filter(or_(User.balance < 5000, User.balance == None))
    # CODE_TRUNCATED )
    {printsql}SELECT "user".id AS user_id, "user".name AS user_name,
    account.balance AS account_balance
...
.. sourcecode:: pycon+sql
    ...
    >>> print(
    # CODE_TRUNCATED     select(sw1.word_insensitive, sw2.word_insensitive).filter(
    # CODE_TRUNCATED         sw1.word_insensitive > sw2.word_insensitive
    # CODE_TRUNCATED     )
    # CODE_TRUNCATED )
...
class _HybridGetterType(Protocol[_T_co]):
    def __call__(s, __self: Any) -> _T_co: # CODE_TRUNCATED


class _HybridSetterType(Protocol[_T_con]):
    def __call__(s, __self: Any, value: _T_con) -> None: # CODE_TRUNCATED
...
class _HybridUpdaterType(Protocol[_T_con]):
    ...
    ) -> List[Tuple[_DMLColumnArgument, Any]]: # CODE_TRUNCATED


class _HybridDeleterType(Protocol[_T_co]):
    def __call__(s, __self: Any) -> None: # CODE_TRUNCATED


class _HybridExprCallableType(Protocol[_T_co]):
    def __call__(
        s, __cls: Any
    ) -> Union[_HasClauseElement[_T_co], SQLColumnExpression[_T_co]]: # CODE_TRUNCATED


class _HybridComparatorCallableType(Protocol[_T]):
    def __call__(self, cls: Any) -> Comparator[_T]: # CODE_TRUNCATED
...
class _HybridClassLevelAccessor(QueryableAttribute[_T]):
    ...
    if TYPE_CHECKING:
        ...
        ) -> hybrid_property[_T]: # CODE_TRUNCATED
        ...
        @property
        def overrides(self) -> hybrid_property[_T]: # CODE_TRUNCATED
...
class hybrid_method(interfaces.InspectionAttrInfo, Generic[_P, _R]):
    ...
    ) -> Callable[_P, SQLCoreOperations[_R]]: # CODE_TRUNCATED

    @overload
    def __get__(
        self, instance: object, owner: Type[object]
    ) -> Callable[_P, _R]: # CODE_TRUNCATED
...
class hybrid_property(interfaces.InspectionAttrInfo, ORMDescriptor[_T]):
    ...
    @overload
    def __get__(self, instance: Any, owner: Literal[None]) -> Self: # CODE_TRUNCATED

    @overload
    def __get__(
        self, instance: Literal[None], owner: Type[object]
    ) -> _HybridClassLevelAccessor[_T]: # CODE_TRUNCATED

    @overload
    def __get__(self, instance: object, owner: Type[object]) -> _T: # CODE_TRUNCATED
