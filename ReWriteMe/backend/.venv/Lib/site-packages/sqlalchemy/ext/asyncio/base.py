...
class ReversibleProxy(Generic[_PT]):
    ...
    @overload
    def _assign_proxied(self, target: _PT) -> _PT: # CODE_TRUNCATED

    @overload
    def _assign_proxied(self, target: None) -> None: # CODE_TRUNCATED
    ...
    def _retrieve_proxy_for_target(
        cls, target: _PT, regenerate: Literal[True] = # CODE_TRUNCATED, **additional_kw: Any
    ) -> Self: # CODE_TRUNCATED
...
class GeneratorStartableContext(StartableContext[_T_co]):
    ...
    def __init__(
        self,
        func: Callable[# CODE_TRUNCATED, AsyncIterator[_T_co]],
        args: Tuple[Any, # CODE_TRUNCATED],
        kwds: Dict[str, Any],
    ):
...
def asyncstartablecontext(
    func: Callable[# CODE_TRUNCATED, AsyncIterator[_T_co]],
) -> Callable[# CODE_TRUNCATED, GeneratorStartableContext[_T_co]]:
