...
class _HasClauseElement(Protocol, Generic[_T_co]):
    ...
    def __clause_element__(self) -> roles.ExpressionElementRole[_T_co]: # CODE_TRUNCATED


class _CoreAdapterProto(Protocol):
    """protocol for the ClauseAdapter/ColumnAdapter.traverse() method."""

    def __call__(self, obj: _CE) -> _CE: # CODE_TRUNCATED
...
class _HasDialect(Protocol):
    ...
    @property
    def dialect(self) -> Dialect: # CODE_TRUNCATED
...
sets; select(# CODE_TRUNCATED), insert().returning(# CODE_TRUNCATED), etc.
...
_TP = TypeVar("_TP", bound=Tuple[Any, # CODE_TRUNCATED])
...
position in a selectable, like TextClause().columns() or values(# CODE_TRUNCATED).
...
if TYPE_CHECKING:
    ...
    def is_sql_compiler(c: Compiled) -> TypeGuard[SQLCompiler]: # CODE_TRUNCATED

    def is_ddl_compiler(c: Compiled) -> TypeGuard[DDLCompiler]: # CODE_TRUNCATED

    def is_named_from_clause(
        t: FromClauseRole,
    ) -> TypeGuard[NamedFromClause]: # CODE_TRUNCATED

    def is_column_element(
        c: ClauseElement,
    ) -> TypeGuard[ColumnElement[Any]]: # CODE_TRUNCATED

    def is_keyed_column_element(
        c: ClauseElement,
    ) -> TypeGuard[KeyedColumnElement[Any]]: # CODE_TRUNCATED

    def is_text_clause(c: ClauseElement) -> TypeGuard[TextClause]: # CODE_TRUNCATED

    def is_from_clause(c: ClauseElement) -> TypeGuard[FromClause]: # CODE_TRUNCATED

    def is_tuple_type(t: TypeEngine[Any]) -> TypeGuard[TupleType]: # CODE_TRUNCATED

    def is_table_value_type(
        t: TypeEngine[Any],
    ) -> TypeGuard[TableValueType]: # CODE_TRUNCATED

    def is_selectable(t: Any) -> TypeGuard[Selectable]: # CODE_TRUNCATED

    def is_select_base(
        t: Union[Executable, ReturnsRows],
    ) -> TypeGuard[SelectBase]: # CODE_TRUNCATED

    def is_select_statement(
        t: Union[Executable, ReturnsRows],
    ) -> TypeGuard[Select[Any]]: # CODE_TRUNCATED

    def is_table(t: FromClause) -> TypeGuard[TableClause]: # CODE_TRUNCATED

    def is_subquery(t: FromClause) -> TypeGuard[Subquery]: # CODE_TRUNCATED

    def is_dml(c: ClauseElement) -> TypeGuard[UpdateBase]: # CODE_TRUNCATED
...
) -> "SQLCoreOperations[Optional[_T]]": # CODE_TRUNCATED


@overload
def Nullable(
    val: roles.ExpressionElementRole[_T],
) -> roles.ExpressionElementRole[Optional[_T]]: # CODE_TRUNCATED


@overload
def Nullable(val: Type[_T]) -> Type[Optional[_T]]: # CODE_TRUNCATED
...
) -> "SQLCoreOperations[_T]": # CODE_TRUNCATED


@overload
def NotNullable(
    val: roles.ExpressionElementRole[Optional[_T]],
) -> roles.ExpressionElementRole[_T]: # CODE_TRUNCATED


@overload
def NotNullable(val: Type[Optional[_T]]) -> Type[_T]: # CODE_TRUNCATED


@overload
def NotNullable(val: Optional[Type[_T]]) -> Type[_T]: # CODE_TRUNCATED
