...
class _CompilerDispatchType(Protocol):
    def __call__(_self, self: Visitable, visitor: Any, **kw: Any) -> Any: # CODE_TRUNCATED
...
class Visitable:
    ...
    if typing.TYPE_CHECKING:
        ...
        def _compiler_dispatch(self, visitor: Any, **kw: Any) -> str: # CODE_TRUNCATED
...
class HasTraverseInternals:
    ...
    def get_children(
        self, *, omit_attrs: Tuple[str, # CODE_TRUNCATED] = (), **kw: Any
...
class _InternalTraversalDispatchType(Protocol):
    def __call__(s, self: object, visitor: HasTraversalDispatch) -> Any: # CODE_TRUNCATED
...
class HasTraversalDispatch:
    ...
    def dispatch(self, visit_symbol: InternalTraversal) -> Callable[# CODE_TRUNCATED, Any]:
...
class ExternallyTraversible(HasTraverseInternals, Visitable):
    ...
    if typing.TYPE_CHECKING:
        ...
        def _annotate(self, values: _AnnotationDict) -> Self: # CODE_TRUNCATED

        def get_children(
            self, *, omit_attrs: Tuple[str, # CODE_TRUNCATED] = (), **kw: Any
        ) -> Iterable[ExternallyTraversible]: # CODE_TRUNCATED
...
class _CloneCallableType(Protocol):
    def __call__(self, element: _ET, **kw: Any) -> _ET: # CODE_TRUNCATED


class _TraverseTransformCallableType(Protocol[_ET]):
    def __call__(self, element: _ET, **kw: Any) -> Optional[_ET]: # CODE_TRUNCATED
...
class ExternalTraversal(util.MemoizedSlots):
    ...
    ) -> ExternallyTraversible: # CODE_TRUNCATED
