...
def visit_binary_product(
    ...
    The function is of the form::
        ...
        def my_fn(binary, left, right): # CODE_TRUNCATED
    ...
    def trunc(self, value: Any) -> str:
        ...
        if lenrep > self.max_chars:
            ...
            rep = (
                ...
                + (
                    " # CODE_TRUNCATED (%d characters truncated) # CODE_TRUNCATED "
...
class _long_statement(str):
    def __str__(self) -> str:
        ...
        if lself > 500:
            ...
            return (
                f"{self[0:lleft]} # CODE_TRUNCATED {trunc} "
                f"characters truncated # CODE_TRUNCATED {self[-lright:]}"
            )
        else:
...
class _repr_params(_repr_base):
    ...
    def __repr__(self) -> str:
        ...
        if self.ismulti:
            ...
            if len(self.params) > self.batches:
                msg = (
                    " # CODE_TRUNCATED displaying %i of %i total bound parameter sets # CODE_TRUNCATED "
                )
                return " ".join(
                    (
                        self._repr_multi(
    ...
    def _repr_param_dict(self, params: _CoreSingleExecuteParams) -> str:
        ...
        if items_second_batch:
            ...
            text += f" # CODE_TRUNCATED {trunclen} parameters truncated # CODE_TRUNCATED "
            text += "%s}" % (
                ", ".join(
                    f"{key!r}: {trunc(value)}"
...
def reduce_columns(
    ...
    **kw: bool,
) -> Sequence[ColumnElement[Any]]: # CODE_TRUNCATED
...
def reduce_columns(
    ...
    *clauses: Optional[ClauseElement],
    **kw: bool,
) -> Sequence[Union[ColumnElement[Any], TextClause]]: # CODE_TRUNCATED
...
class ClauseAdapter(visitors.ReplacingExternalTraversal):
    ...
    if TYPE_CHECKING:
        ...
        @overload
        def traverse(self, obj: Literal[None]) -> None: # CODE_TRUNCATED
        ...
        @overload
        def traverse(self, obj: _ET) -> _ET: # CODE_TRUNCATED

        def traverse(
            self, obj: Optional[ExternallyTraversible]
        ) -> Optional[ExternallyTraversible]: # CODE_TRUNCATED
...
class _ColumnLookup(Protocol):
    @overload
    def __getitem__(self, key: None) -> None: # CODE_TRUNCATED

    @overload
    def __getitem__(self, key: ColumnClause[Any]) -> ColumnClause[Any]: # CODE_TRUNCATED

    @overload
    def __getitem__(self, key: ColumnElement[Any]) -> ColumnElement[Any]: # CODE_TRUNCATED

    @overload
    def __getitem__(self, key: _ET) -> _ET: # CODE_TRUNCATED

    def __getitem__(self, key: Any) -> Any: # CODE_TRUNCATED
...
class ColumnAdapter(ClauseAdapter):
    ...
    * An adapt_required option, used by eager loading to indicate that
    ...
    if TYPE_CHECKING:
        ...
        @property
        def visitor_iterator(self) -> Iterator[ColumnAdapter]: # CODE_TRUNCATED
