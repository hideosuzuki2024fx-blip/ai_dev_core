...
class _LiteralProcessorType(Protocol[_T_co]):
    def __call__(self, value: Any) -> str: # CODE_TRUNCATED


class _BindProcessorType(Protocol[_T_con]):
    def __call__(self, value: Optional[_T_con]) -> Any: # CODE_TRUNCATED


class _ResultProcessorType(Protocol[_T_co]):
    def __call__(self, value: Any) -> Optional[_T_co]: # CODE_TRUNCATED
...
class _ComparatorFactory(Protocol[_T]):
    ...
    ) -> TypeEngine.Comparator[_T]: # CODE_TRUNCATED
...
class TypeEngine(Visitable, Generic[_T]):
    ...
    class Comparator(
        ...
        def operate(
            ...
            **kwargs: Any,
        ) -> ColumnElement[_RT]: # CODE_TRUNCATED

        @overload
        def operate(
            self, op: OperatorType, *other: Any, **kwargs: Any
        ) -> ColumnElement[_CT]: # CODE_TRUNCATED
    ...
    def _static_cache_key(
        self,
    ) -> Union[CacheConst, Tuple[Any, # CODE_TRUNCATED]]:
    ...
    def compile(self, dialect: Optional[Dialect] = None) -> str:
        ...
        # ClauseElement.compile()# CODE_TRUNCATED.this is a mistake.
...
class TypeEngineMixin:
    ...
    if TYPE_CHECKING:
        ...
        def _static_cache_key(
            self,
        ) -> Union[CacheConst, Tuple[Any, # CODE_TRUNCATED]]: # CODE_TRUNCATED
        ...
        def dialect_impl(self, dialect: Dialect) -> TypeEngine[Any]: # CODE_TRUNCATED
...
class ExternalType(TypeEngineMixin):
    ...
    series of tuples.   Given a previously un-cacheable type as::
        ...
        class LookupType(UserDefinedType):
            ...
            def bind_processor(self, dialect): # CODE_TRUNCATED  # works with "self.lookup" # CODE_TRUNCATED
    ...
    to the ".lookup" attribute::
        ...
        class LookupType(UserDefinedType):
            ...
            def bind_processor(self, dialect): # CODE_TRUNCATED  # works with "self._lookup" # CODE_TRUNCATED
...
class UserDefinedType(
    ...
    if TYPE_CHECKING:
        ...
        def get_col_spec(self, **kw: Any) -> str: # CODE_TRUNCATED
...
@overload
def to_instance(typeobj: None, *arg: Any, **kw: Any) -> TypeEngine[None]: # CODE_TRUNCATED
