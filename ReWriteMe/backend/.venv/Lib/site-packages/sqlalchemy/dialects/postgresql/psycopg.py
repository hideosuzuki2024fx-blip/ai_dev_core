...
.. dialect:: postgresql+psycopg
    ...
    :dbapi: psycopg
    :connectstring: postgresql+psycopg://user:password@host:port/dbname[?key=value&key=value# CODE_TRUNCATED]
...
* calling :func:`_sa.create_engine` with ``postgresql+psycopg://# CODE_TRUNCATED`` will
...
* calling :func:`_asyncio.create_async_engine` with
  ``postgresql+psycopg://# CODE_TRUNCATED`` will automatically select the async version,
  e.g.::
...
``psycopg_pool >= 3.3`` that introduces support for ``close_returns=True``::
    ...
    my_pool = psycopg_pool.ConnectionPool(
        ...
        # # CODE_TRUNCATED other pool parameters as desired # CODE_TRUNCATED
    )
...
the engine::
    ...
    client_side_engine = create_engine(
        "postgresql+psycopg://# CODE_TRUNCATED",
