...
To set using per-connection execution options::
    ...
    with engine.connect() as conn:
        ...
        with conn.begin():
            # CODE_TRUNCATED  # work with transaction
...
reflection process as follows::
    ...
    >>> with engine.connect() as conn:
    # CODE_TRUNCATED     conn.execute(text("SET search_path TO test_schema, public"))
    # CODE_TRUNCATED     metadata_obj = MetaData()
    # CODE_TRUNCATED     referring = Table("referring", metadata_obj, autoload_with=conn)
...
:meth:`_schema.MetaData.reflect`::
    ...
    # CODE_TRUNCATED     conn.execute(text("SET search_path TO test_schema, public"))
    # CODE_TRUNCATED     metadata_obj = MetaData()
    # CODE_TRUNCATED     referring = Table(
    # CODE_TRUNCATED         "referring",
    # CODE_TRUNCATED         metadata_obj,
    # CODE_TRUNCATED         autoload_with=conn,
    # CODE_TRUNCATED         postgresql_ignore_search_path=True,
    # CODE_TRUNCATED     )
...
INSERT/UPDATE# CODE_TRUNCATEDRETURNING
-------------------------
...
INSERT# CODE_TRUNCATEDON CONFLICT (Upsert)
------------------------------
...
.. sourcecode:: pycon+sql
    ...
    # CODE_TRUNCATED     id="some_existing_id", data="inserted value"
    # CODE_TRUNCATED )
    ...
    # CODE_TRUNCATED     constraint="pk_my_table", set_=dict(data="updated value")
    # CODE_TRUNCATED )
...
* The :paramref:`_postgresql.Insert.on_conflict_do_update.index_elements` argument
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> do_update_stmt = insert_stmt.on_conflict_do_update(
    # CODE_TRUNCATED     index_elements=["id"], set_=dict(data="updated value")
    # CODE_TRUNCATED )
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, data) VALUES (%(id)s, %(data)s)
    ...
    >>> do_update_stmt = insert_stmt.on_conflict_do_update(
    # CODE_TRUNCATED     index_elements=[my_table.c.id], set_=dict(data="updated value")
    # CODE_TRUNCATED )
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, data) VALUES (%(id)s, %(data)s)
    ON CONFLICT (id) DO UPDATE SET data = %(param_1)s
...
* When using :paramref:`_postgresql.Insert.on_conflict_do_update.index_elements` to
  ...
  .. sourcecode:: pycon+sql
    ...
    # CODE_TRUNCATED     index_elements=[my_table.c.user_email],
    # CODE_TRUNCATED     index_where=my_table.c.user_email.like("%@gmail.com"),
    # CODE_TRUNCATED     set_=dict(data=stmt.excluded.data),
    # CODE_TRUNCATED )
    >>> print(stmt)
...
* The :paramref:`_postgresql.Insert.on_conflict_do_update.constraint` argument is
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> do_update_stmt = insert_stmt.on_conflict_do_update(
    # CODE_TRUNCATED     constraint="my_table_idx_1", set_=dict(data="updated value")
    # CODE_TRUNCATED )
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, data) VALUES (%(id)s, %(data)s)
    ...
    >>> do_update_stmt = insert_stmt.on_conflict_do_update(
    # CODE_TRUNCATED     constraint="my_table_pk", set_=dict(data="updated value")
    # CODE_TRUNCATED )
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, data) VALUES (%(id)s, %(data)s)
...
* The :paramref:`_postgresql.Insert.on_conflict_do_update.constraint` argument may
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> do_update_stmt = insert_stmt.on_conflict_do_update(
    # CODE_TRUNCATED     constraint=my_table.primary_key, set_=dict(data="updated value")
    # CODE_TRUNCATED )
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, data) VALUES (%(id)s, %(data)s)
    ON CONFLICT (id) DO UPDATE SET data = %(param_1)s
...
``ON CONFLICT# CODE_TRUNCATEDDO UPDATE`` is used to perform an update of the already
...
.. sourcecode:: pycon+sql
    ...
    # CODE_TRUNCATED     id="some_id", data="inserted value", author="jlh"
    # CODE_TRUNCATED )
    >>> do_update_stmt = stmt.on_conflict_do_update(
    # CODE_TRUNCATED     index_elements=["id"],
    # CODE_TRUNCATED     set_=dict(data="updated value", author=stmt.excluded.author),
    # CODE_TRUNCATED )
    >>> print(do_update_stmt)
...
.. sourcecode:: pycon+sql
    ...
    # CODE_TRUNCATED     index_elements=["id"],
    # CODE_TRUNCATED     set_=dict(data="updated value", author=stmt.excluded.author),
    # CODE_TRUNCATED     where=(my_table.c.status == 2),
    # CODE_TRUNCATED )
...
It is recommended that you use the ``EXPLAIN ANALYZE# CODE_TRUNCATED`` tool from
...
FROM ONLY # CODE_TRUNCATED
-------------

The dialect supports PostgreSQL's ONLY keyword for targeting only a particular
table in an inheritance hierarchy. This can be used to produce the
``SELECT # CODE_TRUNCATED FROM ONLY``, ``UPDATE ONLY # CODE_TRUNCATED``, and ``DELETE FROM ONLY # CODE_TRUNCATED``
syntaxes. It uses SQLAlchemy's hints mechanism::

    # SELECT # CODE_TRUNCATED FROM ONLY # CODE_TRUNCATED
    result = table.select().with_hint(table, "ONLY", "postgresql")
    print(result.fetchall())

    # UPDATE ONLY # CODE_TRUNCATED
    table.update(values=dict(foo="bar")).with_hint(
        "ONLY", dialect_name="postgresql"
    )

    # DELETE FROM ONLY # CODE_TRUNCATED
...
``Table(# CODE_TRUNCATED, autoload_with=engine)``, the UNIQUE INDEX is **not** returned
...
::
    ...
    Table("some_table", metadata, # CODE_TRUNCATED, postgresql_inherits="some_supertable")

    Table("some_table", metadata, # CODE_TRUNCATED, postgresql_inherits=("t1", "t2", # CODE_TRUNCATED))
...
::
    ...
    Table("some_table", metadata, # CODE_TRUNCATED, postgresql_on_commit="PRESERVE ROWS")
...
::
    ...
    Table("some_table", metadata, # CODE_TRUNCATED, postgresql_tablespace="some_tablespace")
...
::
    ...
    Table("some_table", metadata, # CODE_TRUNCATED, postgresql_using="heap")
...
::
    ...
    Table("some_table", metadata, # CODE_TRUNCATED, postgresql_with_oids=True)
...
::
    ...
    Table("some_table", metadata, # CODE_TRUNCATED, postgresql_with_oids=False)
...
* ``json_each()``:
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> stmt = select(
    # CODE_TRUNCATED     func.json_each('{"a":"foo", "b":"bar"}').table_valued("key", "value")
    # CODE_TRUNCATED )
    >>> print(stmt)
...
* ``json_populate_record()``:
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> stmt = select(
    # CODE_TRUNCATED     func.json_populate_record(
    # CODE_TRUNCATED         literal_column("null::myrowtype"), '{"a":1,"b":2}'
    # CODE_TRUNCATED     ).table_valued("a", "b", name="x")
    # CODE_TRUNCATED )
    >>> print(stmt)
...
* ``json_to_record()`` - this form uses a PostgreSQL specific form of derived
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> stmt = select(
    # CODE_TRUNCATED     func.json_to_record('{"a":1,"b":[1,2,3],"c":"bar"}')
    # CODE_TRUNCATED     .table_valued(
    # CODE_TRUNCATED         column("a", Integer),
    # CODE_TRUNCATED         column("b", Text),
    # CODE_TRUNCATED         column("d", Text),
    # CODE_TRUNCATED     )
    # CODE_TRUNCATED     .render_derived(name="x", with_types=True)
    # CODE_TRUNCATED )
    >>> print(stmt)
...
* ``WITH ORDINALITY`` - part of the SQL standard, ``WITH ORDINALITY`` adds an
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy import select, func
    >>> stmt = select(
    # CODE_TRUNCATED     func.generate_series(4, 1, -1)
    # CODE_TRUNCATED     .table_valued("value", with_ordinality="ordinality")
    # CODE_TRUNCATED     .render_derived()
    # CODE_TRUNCATED )
    >>> print(stmt)
...
* ``json_array_elements()``:
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy import select, func
    >>> stmt = select(
    # CODE_TRUNCATED     func.json_array_elements('["one", "two"]').column_valued("x")
    # CODE_TRUNCATED )
    >>> print(stmt)
    {printsql}SELECT x
...
.. sourcecode:: pycon+sql
    ...
    >>> stmt = (
    # CODE_TRUNCATED     t.select()
    # CODE_TRUNCATED     .where(tuple_(t.c.id, t.c.fk) > (1, 2))
    # CODE_TRUNCATED     .where(func.ROW(t.c.id, t.c.fk) < func.ROW(3, 7))
    # CODE_TRUNCATED )
    >>> print(stmt)
...
from # CODE_TRUNCATED import exc
from # CODE_TRUNCATED import schema
from # CODE_TRUNCATED import select
from # CODE_TRUNCATED import sql
from # CODE_TRUNCATED import util
from # CODE_TRUNCATEDengine import characteristics
from # CODE_TRUNCATEDengine import default
from # CODE_TRUNCATEDengine import interfaces
from # CODE_TRUNCATEDengine import ObjectKind
from # CODE_TRUNCATEDengine import ObjectScope
from # CODE_TRUNCATEDengine import reflection
from # CODE_TRUNCATEDengine import URL
from # CODE_TRUNCATEDengine.reflection import ReflectionDefaults
from # CODE_TRUNCATEDsql import bindparam
from # CODE_TRUNCATEDsql import coercions
from # CODE_TRUNCATEDsql import compiler
from # CODE_TRUNCATEDsql import elements
from # CODE_TRUNCATEDsql import expression
from # CODE_TRUNCATEDsql import roles
from # CODE_TRUNCATEDsql import sqltypes
from # CODE_TRUNCATEDsql import util as sql_util
from # CODE_TRUNCATEDsql.compiler import InsertmanyvaluesSentinelOpts
from # CODE_TRUNCATEDsql.visitors import InternalTraversal
from # CODE_TRUNCATEDtypes import BIGINT
from # CODE_TRUNCATEDtypes import BOOLEAN
from # CODE_TRUNCATEDtypes import CHAR
from # CODE_TRUNCATEDtypes import DATE
from # CODE_TRUNCATEDtypes import DOUBLE_PRECISION
from # CODE_TRUNCATEDtypes import FLOAT
from # CODE_TRUNCATEDtypes import INTEGER
from # CODE_TRUNCATEDtypes import NUMERIC
from # CODE_TRUNCATEDtypes import REAL
from # CODE_TRUNCATEDtypes import SMALLINT
from # CODE_TRUNCATEDtypes import TEXT
from # CODE_TRUNCATEDtypes import UUID as UUID
from # CODE_TRUNCATEDtypes import VARCHAR
from # CODE_TRUNCATEDutil.typing import TypedDict
...
class PGDialect(default.DefaultDialect):
    ...
    def _split_multihost_from_url(self, url: URL) -> Union[
        Tuple[None, None],
        Tuple[Tuple[Optional[str], # CODE_TRUNCATED], Tuple[Optional[int], # CODE_TRUNCATED]],
    ]:
        hosts: Optional[Tuple[Optional[str], # CODE_TRUNCATED]] = None
        ports_str: Union[str, Tuple[Optional[str], # CODE_TRUNCATED], None] = None
            ...
            if isinstance(url.query["host"], (list, tuple)):
                ...
                if (
                    ...
                    if host_port_match:
                        ...
                        hosts = (h,)
                        ports_str = cast(
                            "Tuple[Optional[str], # CODE_TRUNCATED]", (p,) if p else (None,)
                        )
        ...
        ports: Optional[Tuple[Optional[int], # CODE_TRUNCATED]] = None
    ...
    def _kind_to_relkinds(self, kind: ObjectKind) -> Tuple[str, # CODE_TRUNCATED]:
